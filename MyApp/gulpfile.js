(function () {
    ...
    var resourcesDir = '../MyApp.Resources/';
    var winFormsAssemblyInfoPath = '../MyApp.AppWinForms/Properties/AssemblyInfo.cs';
    var consoleDir = '../MyApp.AppConsole/bin/Release/';
    var consoleExeName = 'MyApp-console.exe';
    ...
    gulp.task('www-copy-resources-lib', () => {
        return gulp.src('../MyApp.Resources/bin/Release/MyApp.Resources.dll')
    ...
    gulp.task('www-copy-resources', () => {
        return gulp.src(['./wwwroot/**/*',
            '!./wwwroot/platform.*'])
            .pipe(gulp.dest('../MyApp.Resources/'));
    });
    ...
    gulp.task('www-msbuild-resources', () => {
        return gulp.src('../MyApp.Resources/MyApp.Resources.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-console', () => {
        return gulp.src('../MyApp.AppConsole/MyApp.AppConsole.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-winforms', () => {
        return gulp.src('../MyApp.AppWinforms/MyApp.AppWinforms.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-nuget-pack-winforms', function (callback) {
        ...
        var excludes = globule.find([
            '../MyApp.AppWinForms/bin/x86/Release/*.pdb',
            '../MyApp.AppWinForms/bin/x86/Release/*.vshost.*'
        ]);
        var includes = [
        { src: '../MyApp.AppWinForms/bin/x86/Release/*.*', dest: '/lib/net45/' },
        { src: '../MyApp.AppWinForms/bin/x86/Release/locales/*.*', dest: '/lib/net45/locales/' }];
        return nugetpack({
            id: title,
            title: title,
                version: version,
